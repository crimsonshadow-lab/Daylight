...
function duration(text, fallback) { const m=text.match(/(?:about |around |for )?(\d+(?:\.\d+)?)\s*(hours?|hrs?|hr|minutes?|mins?|min)\b/i); return m?Math.round(Number(m[1])*(/h/i.test(m[2])?60:1)):fallback; }
...
export function interpret(text, now=new Date(), patterns={}) {
  ...
  const commitmentWords=/\b(meeting|lecture|class|appointment|event|busy|call|interview)\b/.test(lower);
  ...
  if(commitmentWords&&!taskWords){const t=parseTime(text);const lowerPriority=/synergy|insightx/.test(lower);return {type:"commitment",value:{id:uid(),title:taskTitle(text)||"Commitment",date:date||iso(now),start:t?.start||"18:00",end:t?.end||"19:00",hard:true,priority:lowerPriority?"low":category==="SCOP"?"high":"normal",source:"user"},notes:[]};}
  ...
  const priority=/urgent|asap|tomorrow|today/.test(lower)?"high":category==="SCOP"||category==="Academic"?"high":category==="Study"?"medium":"low";
