...
function taskRank(task,today) {
  ...
  // Club priority is explicitly SCOP > Synergy > InsightX (spec section 7); these must resolve to distinct
  // scores or ties fall back to array insertion order, which silently defeats the ordering entirely.
  if(task.category==="SCOP") score+=180;
  else if(task.category==="Synergy") score-=100;
  else if(task.category==="InsightX") score-=240;
...
// Dinner is a flexible 45-minute meal placed inside the mess window (default 20:00-22:00), never a fixed
// appointment. It must find an actual free gap around that day's hard commitments rather than always sitting
// at dinnerStart regardless of what else is happening then. Returns null when no valid gap exists so the
// caller can surface an explicit conflict instead of silently emitting an overlapping block.
function dinnerSlot(workspace,date) {
  const p=workspace.preferences, winStart=mins(p.dinnerStart), winEnd=mins(p.dinnerEnd), duration=Number(p.dinnerMinutes||45);
  const commitmentRanges=workspace.commitments.filter(c=>c.date===date).map(c=>[mins(c.start),mins(c.end)]);
  const gaps=subtract([[winStart,winEnd]],commitmentRanges).filter(([a,b])=>b-a>=duration);
  if(!gaps.length) return null;
  const [a]=gaps[0];
  return {id:`dinner-${date}`,title:"Dinner",category:"Personal",date,start:clock(a),end:clock(a+duration),minutes:duration,kind:"dinner"};
}
...
export function plan(workspace, now=new Date()) {
  ...
  // A single overloaded day is enough to ease the day(s) right after it (spec section 13's own example is
  // exactly one hectic Monday easing Tuesday); requiring two hectic days in the recent window meant this
  // never actually triggered from ordinary single-day overload.
  const recoveryHigh=workspace.recovery?.load==="high"||((workspace.recovery?.hecticDates||[]).filter(d=>dayDiff(today,d)>=0&&dayDiff(today,d)<=3).length>=1);
  for(let offset=0;offset<days;offset++) {
    ...
    if(new Date(`${date}T12:00`).getDay()!==0) {
      const dinner=dinnerSlot(workspace,date);
      if(dinner) blocks.push(dinner);
      else prompts.push({id:`dinner-conflict-${date}`,level:"ask",text:`No ${Number(workspace.preferences.dinnerMinutes||45)}-minute gap is free in the ${workspace.preferences.dinnerStart}\u2013${workspace.preferences.dinnerEnd} dinner window on ${date}. Please adjust that day's commitments or widen the dinner window in Settings.`});
    }
  }
  ...
  // Defensive validation (never trust generated data blindly): a block where end <= start is always a bug,
  // not a representable overnight event, since none of the generators here support crossing midnight. Drop
  // any that somehow slip through rather than ever rendering an invalid range in the UI.
  const safeBlocks=blocks.filter(b=>{const invalid=mins(b.end)<=mins(b.start); if(invalid&&typeof console!=="undefined") console.warn("Daylight: dropped invalid generated block (end<=start)",b); return !invalid;});
  workspace.plan=safeBlocks;workspace.prompts=prompts;workspace.recovery={...workspace.recovery,load:recoveryHigh?"moderate":"normal",lastUpdated:new Date().toISOString()};return workspace;
}
